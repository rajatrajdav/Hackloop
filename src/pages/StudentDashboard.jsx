...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sd-nav-btn:hover { background: rgba(244,160,35,.07) !important; color: var(--ink) !important; }
  .sd-row:hover { background: #fdfcfa !important; }
  .sd-stat-card { transition: box-shadow .2s, transform .2s; }
  .sd-stat-card:hover { box-shadow: var(--shadow-md); transform: translateY(-2px); }
...
export default function StudentDashboard({ user, onLogout }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--cream)", fontFamily: "var(--ff-body)", color: "var(--ink)" }}>
      ...
      {/* Sidebar */}
      <aside style={{ width: 224, background: "#fff", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", boxShadow: "1px 0 0 var(--border)" }}>
        <div style={{ padding: "24px 20px 18px" }}>
          <img src={logo} alt="Sangam" style={{ height: 32, width: "auto", objectFit: "contain" }} />
          <div style={{ marginTop: 10, display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(244,160,35,.1)", color: "#b06a00", borderRadius: 6, padding: "3px 10px", fontSize: "0.65rem", fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase" }}>Student Portal</div>
        </div>
        ...
        <nav style={{ flex: 1, padding: "14px 10px", display: "flex", flexDirection: "column", gap: 1 }}>
          {NAV.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className="sd-nav-btn"
              ...
              {item.id === "myevents" && joinedEvents.size > 0 && (
                <span style={{ marginLeft: "auto", background: "var(--saffron)", color: "#fff", borderRadius: 20, padding: "1px 7px", fontSize: "0.65rem", fontWeight: 800 }}>{joinedEvents.size}</span>
              )}
            </button>
          ))}
        </nav>
        ...
        <div style={{ padding: "14px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg,var(--saffron),var(--saffron-g))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.75rem", fontWeight: 800, color: "#fff", flexShrink: 0 }}>{user?.name?.[0] || "A"}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: "0.8rem", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user?.name || "Student"}</div>
              <div style={{ fontSize: "0.68rem", color: "var(--muted)" }}>Student</div>
            </div>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 32px", borderBottom: "1px solid var(--border)", background: "rgba(255,255,255,.95)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 10 }}>
          <div>
            <h1 style={{ fontFamily: "var(--ff-display)", fontWeight: 900, fontSize: "1.4rem", color: "var(--ink)", letterSpacing: "-.01em" }}>{pageTitle}</h1>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "0 14px", gap: 7 }}>
              ...
              <input style={{ background: "transparent", border: "none", outline: "none", color: "var(--ink)", fontSize: "0.83rem", padding: "8px 0", width: 170, fontFamily: "var(--ff-body)" }} placeholder="Search events…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            <button style={{ width: 36, height: 36, borderRadius: 10, border: "1px solid var(--border)", background: "#fff", cursor: "pointer", fontSize: "0.85rem", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
              🔔
              <span style={{ position: "absolute", top: 6, right: 6, width: 6, height: 6, borderRadius: "50%", background: "#c42050", border: "1.5px solid #fff" }}/>
            </button>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,var(--saffron),var(--saffron-g))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", fontWeight: 800, color: "#fff", cursor: "pointer" }}>{user?.name?.[0] || "A"}</div>
          </div>
        </header>
...
>
          ...
          {activeTab === "discover" && (
            <div style={{ animation: "fadeUp .4s ease both" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
                {[
                  { label: "Events Joined", value: joinedEvents.size, color: "var(--saffron)", icon: "🎟" },
                  { label: "Saved Events",  value: savedEvents.size,  color: "#c42050",        icon: "♥" },
                  { label: "Near You",      value: nearbyEvents.length, color: "var(--indigo)", icon: "📍" },
                  { label: "Total Events",  value: events.length,     color: "var(--sage)",    icon: "✦" },
                ].map((s, i) => (
                  <div key={i} className="sd-stat-card" style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 14, padding: "18px 20px", borderTop: `3px solid ${s.color}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: "0.9rem", opacity: .5 }}>{s.icon}</span>
                    </div>
                ...
                <div style={{ display: "flex", gap: 6, background: "var(--surface)", borderRadius: 10, padding: 4 }}>
                  {categories.map(c => (
                    <button key={c} onClick={() => setSelectedCategory(c)}
                      style={{ padding: "6px 16px", borderRadius: 8, border: "none", background: selectedCategory === c ? "#fff" : "transparent", color: selectedCategory === c ? "var(--ink)" : "var(--muted)", cursor: "pointer", fontSize: "0.78rem", fontFamily: "var(--ff-body)", fontWeight: selectedCategory === c ? 700 : 500, transition: "all .18s", boxShadow: selectedCategory === c ? "var(--shadow-sm)" : "none" }}>
                      {c}
                    </button>
                  ))}
                </div>
                <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                  style={{ background: "#fff", border: "1px solid var(--border)", color: "var(--muted)", borderRadius: 10, padding: "7px 14px", fontSize: "0.78rem", outline: "none", fontFamily: "var(--ff-body)", cursor: "pointer" }}>
          ...
          {activeTab === "myevents" && (
            <div style={{ animation: "fadeUp .4s ease both" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <h2 style={{ fontFamily: "var(--ff-display)", fontSize: "1.2rem", fontWeight: 700 }}>Registered Events</h2>
                <span style={{ fontSize: "0.72rem", color: "var(--muted)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "3px 12px" }}>{joinedEvents.size} events</span>
              </div>

              {events.filter(e => joinedEvents.has(e.id)).length === 0 ? (
                <div style={{ textAlign: "center", padding: "64px 0", background: "#fff", borderRadius: 16, border: "1px solid var(--border)" }}>
                  <div style={{ fontSize: "2.5rem", marginBottom: 14 }}>🎟</div>
                  <p style={{ color: "var(--muted)", marginBottom: 18, fontSize: "0.88rem" }}>No events yet. Go discover and join some!</p>
                  <button onClick={() => setActiveTab("discover")} style={{ padding: "10px 24px", background: "var(--ink)", color: "var(--cream)", border: "none", borderRadius: 10, fontFamily: "var(--ff-body)", fontWeight: 700, cursor: "pointer", fontSize: "0.83rem" }}>Explore Events →</button>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 32 }}>
                  {events.filter(e => joinedEvents.has(e.id)).map(ev => (
                    <div key={ev.id} className="sd-row" style={{ display: "flex", alignItems: "center", gap: 14, background: "#fff", border: "1px solid var(--border)", borderRadius: 14, padding: "14px 18px", transition: "background .15s" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: "0.72rem", color: "var(--muted)", marginTop: 3 }}>📅 {formatDate(ev.date)} · 📍 {ev.location}</div>
                      </div>
                      <span style={{ background: "rgba(23,136,90,.08)", color: "var(--sage)", borderRadius: 8, padding: "3px 12px", fontSize: "0.7rem", fontWeight: 700, border: "1px solid rgba(23,136,90,.15)", whiteSpace: "nowrap" }}>Confirmed</span>
                      <button onClick={() => toggleJoin(ev.id)} style={{ padding: "6px 14px", border: "1px solid var(--border)", borderRadius: 8, background: "transparent", color: "var(--muted)", cursor: "pointer", fontSize: "0.75rem", fontFamily: "var(--ff-body)" }}>Cancel</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <h2 style={{ fontFamily: "var(--ff-display)", fontSize: "1.2rem", fontWeight: 700 }}>Saved Events</h2>
                <span style={{ fontSize: "0.72rem", color: "var(--muted)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "3px 12px" }}>{savedEvents.size} saved</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {events.filter(e => savedEvents.has(e.id)).map(ev => (
                  <div key={ev.id} className="sd-row" style={{ display: "flex", alignItems: "center", gap: 14, background: "#fff", border: "1px solid var(--border)", borderRadius: 14, padding: "14px 18px", transition: "background .15s" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: "0.88rem", fontFamily: "var(--ff-display)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{ev.title}</div>
                      <div style={{ fontSize: "0.72rem", color: "var(--muted)", marginTop: 3 }}>📅 {formatDate(ev.date)} · 📍 {ev.location} · {ev.capacity - ev.registered} spots left</div>
                    </div>
                    <button onClick={() => toggleJoin(ev.id)} style={{ padding: "7px 18px", background: joinedEvents.has(ev.id) ? "transparent" : "var(--ink)", color: joinedEvents.has(ev.id) ? "var(--muted)" : "var(--cream)", border: joinedEvents.has(ev.id) ? "1px solid var(--border)" : "none", borderRadius: 8, cursor: "pointer", fontSize: "0.75rem", fontFamily: "var(--ff-body)", fontWeight: 700 }}>
                      {joinedEvents.has(ev.id) ? "Registered ✓" : "Register →"}
                    </button>
                    <button onClick={() => toggleSave(ev.id)} style={{ padding: "6px 12px", border: "1px solid var(--border)", borderRadius: 8, background: "transparent", color: "var(--muted)", cursor: "pointer", fontSize: "0.75rem", fontFamily: "var(--ff-body)" }}>Remove</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* PROFILE */}
          {activeTab === "profile" && (
            <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 20, animation: "fadeUp .4s ease both" }}>
              <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 18, overflow: "hidden" }}>
                <div style={{ height: 80, background: "linear-gradient(135deg,var(--saffron),var(--saffron-g))" }}/>
                <div style={{ textAlign: "center", padding: "0 22px 24px" }}>
                  <div style={{ width: 70, height: 70, borderRadius: "50%", background: "linear-gradient(135deg,var(--saffron),var(--saffron-g))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem", fontWeight: 900, color: "#fff", margin: "-35px auto 0", border: "3px solid #fff", boxShadow: "var(--shadow-sm)" }}>{user?.name?.[0] || "A"}</div>
                  <h2 style={{ fontFamily: "var(--ff-display)", fontWeight: 900, fontSize: "1.1rem", marginTop: 12 }}>{user?.name || "Student"}</h2>
                  <p style={{ fontSize: "0.75rem", color: "var(--muted)", marginBottom: 20 }}>{user?.email || ""}</p>
                  <div style={{ display: "flex", borderTop: "1px solid var(--border)" }}>
                    {[["Joined", joinedEvents.size], ["Saved", savedEvents.size], ["Interests", selectedInterests.size]].map(([l, v], i) => (
                      <div key={l} style={{ flex: 1, padding: "14px 0", textAlign: "center", borderRight: i < 2 ? "1px solid var(--border)" : "none" }}>
                        <div style={{ fontFamily: "var(--ff-display)", fontSize: "1.3rem", fontWeight: 700, color: "var(--saffron-g)" }}>{v}</div>
              ...
              <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 18, padding: 26 }}>
                <h3 style={{ fontFamily: "var(--ff-display)", fontWeight: 700, fontSize: "1.05rem", marginBottom: 18 }}>Account Details</h3>
                {[["Full Name", profile?.name || user?.name || ""], ["Email", profile?.email || user?.email || ""], ["Phone", profile?.phone || ""], ["Institution", profile?.institution || ""], ["City", profile?.city || ""]].map(([l, v]) => (
                  <div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid var(--border)", fontSize: "0.85rem" }}>
                    <span style={{ color: "var(--muted)", fontWeight: 500, fontSize: "0.78rem" }}>{l}</span>
                    <span style={{ fontWeight: 600 }}>{v || <span style={{ color: "var(--border)" }}>—</span>}</span>
                  </div>
                ))}
                <button onClick={async () => {
                  ...
                  }} style={{ marginTop: 22, width: "100%", padding: "11px", background: "var(--ink)", border: "none", borderRadius: 10, color: "var(--cream)", cursor: "pointer", fontWeight: 700, fontSize: "0.85rem", fontFamily: "var(--ff-body)", letterSpacing: ".02em" }}>Edit Profile →</button>
              </div>
            </div>
          )}
